...
from ksp_bootstraps.bootstrap import BootstrapProtocol, XcodeProjectDelegate
...
class KivyXcodeBuilder:
    ...
    def __init__(self, pyproject: PyProjectTomlProtocol, delegate: XcodeProjectDelegate) -> None:
        ...
        if apple_data is None: raise Exception("no apple data")
        ios = apple_data.ios
        macos = apple_data.macos
        if ios is not None:
    ...
    def sync_site_xcframeworks(self) -> None:
        ...
        self.delegate.xcode_generate(
            spec_path=spec_path, project_dir=self.project_dir
        )
    ...
    def generate(self, platforms: list[str] | None = None) -> Path:
        ...
        #runner = XcodeGenRunner()
        #runner.generate(spec_path=spec_path, project_dir=self.project_dir)
        self.delegate.xcode_generate(spec_path=spec_path, project_dir=self.project_dir)
